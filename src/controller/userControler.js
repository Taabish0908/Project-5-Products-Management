...
const createUser = async function (req, res) {
  try {
    ...
    let { fname, lname, email, password, phone, address } =
      data; /*Destructring method*/
    let files = req.files;
    let profileImage;

    if (
      [fname, lname, email, password, phone, address].some(
        (field) => field?.trim() === ""
      )
    ) {
      return res
        .status(400)
        .send({ status: false, message: "All feilds are required" });
    }
    ...
    if (!/^([+]\d{2})?\d{10}$/.test(phone.trim())) {
      return res
        ...
        .send({
          status: false,
          message:
            "Sorry!!! Phone number not valid, please provide valid indian phone number",
        });
    }
    if (!isValid(address.shipping.street)) {
      return res
        .status(400)
        .send({ status: false, message: "Shipping street required" });
    }
    if (!isValid(address.shipping.city)) {
      return res
        .status(400)
        .send({ status: false, message: "Shipping city required" });
    }
    ...
    if (!/^([+]\d{2})?\d{6}$/.test(address.shipping.pincode.trim())) {
      return res
        ...
        .send({
          status: false,
          message:
            "Shipping PinCode not valid, please provide 6 Digit valid pinCode",
        });
    }
    if (!isValid(address.billing.street)) {
      return res
        .status(400)
        .send({ status: false, message: "billing street required" });
    }
    if (!isValid(address.billing.city)) {
      return res
        .status(400)
        .send({ status: false, message: "billing city required" });
    }
    ...
    if (!/^([+]\d{2})?\d{6}$/.test(address.billing.pincode.trim())) {
      return res
        ...
        .send({
          status: false,
          message:
            "Billing PinCode not valid, please provide 6 Digit valid pinCode",
        });
    }
    ...
    if (!(password.trim().length >= 8 && password.trim().length <= 15)) {
      return res
        ...
        .send({
          status: false,
          message: "Please provide password with min 8 and max 14 characters",
        });
    }
    if (!files.length > 0) {
      return res
    ...
    hash = bcrypt.hashSync(
      password,
      saltRounds
    ); /* using bcrypt with 2 arguments pass & saltrounds as 10*/
    ...
    res
      .status(201)
      .send({ status: true, message: "User Registered", data: savedUser });
  } catch (error) {
...
const userLogin = async function (req, res) {
  try {
    ...
    if (isValidRequestBody(query)) {
      return res
        .status(400)
        .send({ status: false, message: "this is not allowed" });
    }
    ...
    if (!isValid(email)) {
      return res
        .status(401)
        .send({ status: false, message: "please input valid emailId" });
    }

    if (!isValid(password)) {
      return res
        .status(401)
        .send({ status: false, message: "please input valid password" });
    }
    ...
    if (user) {
      ...
      return res
        .status(400)
        .send({ status: false, message: "mail id is not valid" });
    }
  } catch (err) {
...
const getUser = async function (req, res) {
  try {
    ...
    if (!isValidObjectId(userId)) {
      return res
        .status(400)
        .send({ status: false, message: "Sorry!!! the user id is not valid" });
    }
    let getUser = await userModel.findById(userId);
    if (!getUser) {
      return res
        .status(404)
        .send({ status: false, message: "Sorr!!! No user with this Id found" });
    }
  ...
  try {
    ...
    if (!checkuser) {
      return res
        ...
        .send({
          status: true,
          message: "Sorry!!! No User found with the given user id",
        });
    }
    ...
    if (!(isValidRequestBody(req.body) || file)) {
      return res
        .status(400)
        .send({ status: false, message: "enter data to update" });
    }
    if (isValid(fname)) {
    ...
    if (isValid(email)) {
      ...
      if (findMail) {
        return res
          ...
          .send({
            status: false,
            message: "Sorry!!! this email is already register",
          });
      }
    ...
    if (isValid(phone)) {
      ...
      if (findPhone) {
        return res
          ...
          .send({
            status: false,
            message: "Sorry!!! this mobile number is already register",
          });
      }
    ...
    if (isValid(password)) {
      if (!password.length >= 8 && password.length <= 15) {
        return res
          ...
          .send({
            status: false,
            message: "password length should be 8 to 15",
          });
      }
    ...
    if (address) {
      ...
      if (address.shipping) {
        ...
        if (address.shipping.pincode) {
          ...
          //   }
          if (
            !/(^[0-9]{6}(?:\s*,\s*[0-9]{6})*$)/.test(address.shipping.pincode)
          ) {
            return res
              .status(400)
              .send({ status: false, msg: `pincode six digit number` });
          }
      ...
      if (address.billing) {
        ...
        if (address.billing.pincode) {
          ...
          //   }
          if (
            !/(^[0-9]{6}(?:\s*,\s*[0-9]{6})*$)/.test(address.billing.pincode)
          ) {
            return res
              .status(400)
              .send({ status: false, message: "pincode six digit number" });
          }
    ...
    if (!updateProfile) {
      return res
        .status(404)
        .send({ status: false, msg: "user profile not found" });
    }
    return res
      .status(200)
      .send({ status: true, msg: "User Profile updated", data: updateProfile });
  } catch (err) {
    res.status(500).send({ status: false, error: err.message });
  }
};
